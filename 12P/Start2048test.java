 ...
 * @author cs415 and ???
 */
public class Start2048test implements KeyListener
{  
    private int [ ][ ]  matrix; 
    private Random gen;
    private TextBox tb;
    ...
    private void mergeLeft( )
    {
      ...
      for( int i = 0; i < matrix.length; i++ )
     {
      for( int j = 0; j < 4; j ++ )
      {
       for( int r = matrix[ i ].length - 1; r > - 1 ; r-- )
       {
        if( r < 3 )
        {
         if( matrix[ i ][ r + 1 ] == 0 )
      ...
      for( int j = 0; j < 4; j ++ )
      {
       for( int r = matrix[ i ].length - 1; r > - 1 ; r-- )
       {
        if( r < 3 )
        {
         if( matrix[ i + 1 ][ r ] == 0 )
         {
          matrix[ i + 1 ][ r ] = matrix[ i ][ r ];
             matrix[ i ][ r ] = 0;
         }
         
         if( matrix[ i + 1 ][ r ] == matrix[ i ][ r ] )
            {
             matrix[ i + 1 ][ r  ] = ( matrix[ i + 1 ][ r ] * 2 );
             matrix[ i ][ r ] = 0;
            }
